...
import com.fs.starfarer.stubs.java.util.zip.DataFormatException;
import com.fs.starfarer.stubs.java.util.zip.Deflater;
import com.fs.starfarer.stubs.java.util.zip.Inflater;

import com.fs.starfarer.stubs.javax.xml.bind.DatatypeConverter;
...
//		}
		...
		for (int i = 0; i < tiles.length; i++) {
			for (int j = 0; j < tiles[0].length; j++) {
				System.out.print(Misc.format("% 2d,", tiles[i][j]));
			}
			System.out.println();
		}
		...
		for (int i = 0; i < tilesBack.length; i++) {
			for (int j = 0; j < tilesBack[0].length; j++) {
				System.out.print(Misc.format("% 2d,", tilesBack[i][j]));
			}
			System.out.println();
		}
