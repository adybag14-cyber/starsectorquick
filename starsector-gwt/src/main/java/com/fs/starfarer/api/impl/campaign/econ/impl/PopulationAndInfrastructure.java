...
//	}
	...
	public static void modifyStability2(Industry industry, MarketAPI market, String modId) {
		if (Misc.getNumIndustries(market) > Misc.getMaxIndustries(market)) {
			market.getStability().modifyFlat("var_underscore" + modId + "_overmax", -Misc.OVER_MAX_INDUSTRIES_PENALTY, "Maximum number of industries exceeded");
		} else {
			market.getStability().unmodifyFlat("var_underscore" + modId + "_overmax");
		}
	}
	...
	public static void modifyStability(Industry industry, MarketAPI market, String modId) {
		...
		market.getStability().modifyFlat("var_underscore" + modId + "_ms", Global.getSettings().getFloat("stabilityBaseValue"), "Base value");
		...
		if (market.isPlayerOwned() && market.getAdmin().isPlayer()) {
			...
			if (penalty > 0) {
				market.getStability().modifyFlat("var_underscore" + modId + "_mm", -penalty, "Mismanagement penalty");
			} else if (penalty < 0) {
				market.getStability().modifyFlat("var_underscore" + modId + "_mm", -penalty, "Management bonus");
			} else {
				market.getStability().unmodifyFlat("var_underscore" + modId + "_mm");
			}
		} else {
...
//		}
		market.getStability().unmodifyFlat("var_underscore" + modId + "_mm");
		market.getStability().unmodifyFlat("var_underscore" + modId + "_ms");
		market.getStability().unmodifyFlat("var_underscore" + modId + "_overmax");
