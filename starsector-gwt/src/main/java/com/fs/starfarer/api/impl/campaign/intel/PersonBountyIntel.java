...
public class PersonBountyIntel extends BaseIntelPlugin implements EveryFrameScript, FleetEventListener {
	...
	public PersonBountyIntel() {
		...
		log.info(Misc.format("Starting person bounty by faction [%s] for person %s", faction.getDisplayName(), person.getName().getFullName()));
...
//		}
//		
//		String targetDesc = Misc.format("%s is in command of a %s fleet and was last seen using a %s as %s flagship.",
//						person.getName().getFullName(), fleetDesc, shipType, hisOrHer);					
//		
//		if (skillDesc.isEmpty()) {
//			targetDesc += Misc.format(" %s is known to be %s.", Misc.ucFirst(heOrShe), levelDesc);
//		} else {
//			targetDesc += Misc.format(" %s is %s known for %s.", Misc.ucFirst(heOrShe), levelDesc, skillDesc);
//		}
//		
//		//targetDesc += "\n\nLevel: " + level;
//		
...
//	}
	...
	protected void advanceImpl(float amount) {
		...
		if (elapsedDays >= duration && !isDone()) {
			...
			if (canEnd) {
				log.info(Misc.format("Ending bounty on %s by %s", person.getName().getFullName(), faction.getDisplayName()));
	...
	public void reportBattleOccurred(CampaignFleetAPI fleet, CampaignFleetAPI primaryWinner, BattleAPI battle) {
		...
		if (willPay()) {
			log.info(Misc.format("Paying bounty of %d from faction [%s]", (int) payment, faction.getDisplayName()));
			...
			log.info(Misc.format("Not paying bounty, but improving rep with faction [%s]", faction.getDisplayName()));
			ReputationAdjustmentResult rep = Global.getSector().adjustPlayerReputation(
								new RepActionEnvelope(RepActions.PERSON_BOUNTY_REWARD, null, null, null, true, false), 
								faction.getId());
			...
			log.info(Misc.format("Not paying bounty or improving rep with faction [%s]", faction.getDisplayName()));
