...
public class BattleAutoresolverPluginImpl implements BattleAutoresolverPlugin {
	...
	public static class FleetAutoresolveData {
		...
		public void report() {
			...
			BattleAutoresolverPluginImpl.report(Misc.format("Fighting srength of %s: %f", fleet.getNameWithFaction(), fightingStrength));
			for (FleetMemberAutoresolveData data : members) {
				String str = Misc.format("%40s: CR % 3d%%    FP % 4d     STR % 3f   Shield %3.2f",
	...
	public void resolve() {
		...
		report("***");
		report("***");
		report(Misc.format("Autoresolving %s vs %s", one.getNameWithFaction(), two.getNameWithFaction()));
		...
		if (optionOne == EncounterOption.DISENGAGE && optionTwo == EncounterOption.ENGAGE) {
			report(Misc.format("%s wants to disengage", one.getNameWithFaction()));
			oneEscaping = true;
			if (freeDisengageIfCanOutrun && context.canOutrunOtherFleet(one, two)) {
				report(Misc.format("%s can outrun other fleet", one.getNameWithFaction()));
				report("Finished autoresolving engagement");
				report("***");
				report("***");
				return;
		...
		if (optionOne == EncounterOption.ENGAGE && optionTwo == EncounterOption.DISENGAGE) {
			report(Misc.format("%s wants to disengage", two.getNameWithFaction()));
			twoEscaping = true;
			if (freeDisengageIfCanOutrun && context.canOutrunOtherFleet(two, one)) {
				report(Misc.format("%s can outrun other fleet", two.getNameWithFaction()));
				report("Finished autoresolving engagement");
				report("***");
				report("***");
				return;
	...
	protected void resolveEngagement(FleetEncounterContext context, boolean oneEscaping, boolean twoEscaping) {
		...
		if ((dataOne.fightingStrength > dataTwo.fightingStrength || twoEscaping) && !oneEscaping) {
			report(Misc.format("%s won engagement", one.getNameWithFaction()));
			winner = dataOne;
			loser = dataTwo;
			if (twoEscaping) {
				loserEscaping = true;
			...
			report(Misc.format("%s won engagement", two.getNameWithFaction()));
			winner = dataTwo;
			loser = dataOne;
			if (oneEscaping) {
				loserEscaping = true;
...
//		if (winnerAdvantage < 0.5f) winnerAdvantage = 0.5f;
		...
		for (FleetMemberAutoresolveData data : loser.members) {
			report(Misc.format("Remaining damage to loser: %02.2f", damageDealtToLoser));
		...
		for (FleetMemberAutoresolveData data : winner.members) {
			...
			report(Misc.format("Remaining damage to winner: %02.2f", damageDealtToWinner));
...
//		context.getBattle().finish();
	...
	protected FleetMemberBattleOutcome computeOutcomeForFleetMember(FleetMemberAutoresolveData data, float advantageInBattle, 
		...
		report(Misc.format("Disabled: %d, Heavy: %d, Medium: %d, Light: %d, Unscathed: %d (Shield ratio: %3.2f)",
		...
		switch (outcome) {
		case DISABLED:
			report(Misc.format("%40s: disabled", data.member.getVariant().getFullDesignationWithHullName()));
			damage = 1f;
			break;
		case HEAVY_DAMAGE:
			report(Misc.format("%40s: heavy damage", data.member.getVariant().getFullDesignationWithHullName()));
			damage = 0.7f + (float) Math.random() * 0.1f;
			break;
		case MEDIUM_DAMAGE:
			report(Misc.format("%40s: medium damage", data.member.getVariant().getFullDesignationWithHullName()));
			damage = 0.45f + (float) Math.random() * 0.1f;
			break;
		case LIGHT_DAMAGE:
			report(Misc.format("%40s: light damage", data.member.getVariant().getFullDesignationWithHullName()));
			damage = 0.2f + (float) Math.random() * 0.1f;
			break;
		case UNSCATHED:
			report(Misc.format("%40s: unscathed", data.member.getVariant().getFullDesignationWithHullName()));
			damage = 0f;
			break;
		}
