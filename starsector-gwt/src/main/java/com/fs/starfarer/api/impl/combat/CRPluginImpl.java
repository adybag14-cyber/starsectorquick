...
//		}
	...
	public CREffectDescriptionForTooltip getCREffectDescription(float cr, String shipOrWing, FleetMemberAPI member) {
		...
		String str;
		if (cr < noDeploy) {
			str = Misc.format("The %s is not ready for combat and can not be deployed in battle.", shipOrWing);
		} else if (cr < getCriticalMalfunctionThreshold(member.getStats())) {
			str = Misc.format("The %s suffers from degraded performance and runs the risk of permanent and damaging malfunctions if deployed.", shipOrWing);
		} else if (cr < getMalfunctionThreshold(member.getStats())) {
			str = Misc.format("The %s suffers from degraded performance and runs the risk of weapon and engine malfunctions during combat.", shipOrWing);
		} else if (cr < getDegradeThreshold(member.getStats()) && hasNegative) {
			str = Misc.format("The %s suffers from degraded performance during combat.", shipOrWing);
		} else if (cr < getImproveThreshold(member.getStats()) || !hasPositive) {
			str = Misc.format("The %s has standard combat performance.", shipOrWing);
		} else {
			str = Misc.format("The %s benefits from improved combat performance.", shipOrWing);
		}
