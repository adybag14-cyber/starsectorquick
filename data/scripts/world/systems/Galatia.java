...
public class Galatia {
	...
	public void generate(SectorAPI sector) {
		...
		// Web runtime can miss procgen specs (e.g. star_yellow) during jump-point autogen.
		// Fail-open so campaign generation can continue instead of bouncing back to title.
		try {
			system.autogenerateHyperspaceJumpPoints(true, false);
		} catch (Throwable t) {
			try {
				Global.getLogger(Galatia.class).warn("Skipping Galatia jump-point autogen", t);
			} catch (Throwable ignored) {
			}
		}
...
//		market.getMemoryWithoutUpdate().set(DecivTracker.NO_DECIV_KEY, true);
	...
	protected void addDerelict(StarSystemAPI system, SectorEntityToken focus, String variantId, 
		...
		try {
			...
			try {
				Global.getLogger(Galatia.class).warn("Skipping Galatia derelict variant: " + variantId, t);
			} catch (Throwable ignored) {
			}
		}
	}

	protected boolean shouldSkipTutorialDerelicts() {
		String force = System.getProperty("starsector.skipGalatiaDerelicts", "");
