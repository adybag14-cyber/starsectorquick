...
ORDER = ['baseline-a', 'pointer-matrix', 'baseline-b']
BASELINE_SHA = '457d36910eea99e83518f936db17b1d2a8420617'
CANDIDATE_SHA = '888c0995b7e14c65f99701ef78aba4fa977684ac'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            matrix_inplace=bool(gp.get('matrixInPlaceObserved')),
...
def main():
    ...
    a, c, b = by['baseline-a'], by['pointer-matrix'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Pointer-dirty production vs pointer-dirty + matrix in-place same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | matrix in-place | draws | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('matrix_inplace','-')} | {r.get('interleaved_draws','-')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted pointer-matrix delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('pointer-matrix', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if draws < 1000 or int(c.get('interleaved_uploads') or 0) < 1000:
        raise SystemExit(f'matrix-inplace traffic evidence invalid: {c}')
    if c.get('matrix_inplace') is not True:
        raise SystemExit(f'matrix-inplace candidate did not activate: {c}')
    if a.get('matrix_inplace') or b.get('matrix_inplace'):
        raise SystemExit(f'production baseline unexpectedly reports matrix in-place activation: A={a} B={b}')
    return 0
