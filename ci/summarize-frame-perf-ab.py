...
ORDER = ['baseline-a', 'direct-input', 'baseline-b']
BASELINE_SHA = 'a4c20a8e4b5abb5ccb2e470ad5479b3387750155'
CANDIDATE_SHA = '3544c7ebbe9a14825389ec6a862459e671b1dade'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def main():
    ...
    a, c, b = by['baseline-a'], by['direct-input'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production vs direct-input no-op poll same-runner A/B', '',
    ...
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted direct-input delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('direct-input', CANDIDATE_SHA)]:
        r = by[name]
