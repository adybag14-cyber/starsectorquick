...
ORDER = ['baseline-a', 'vertex-attrib', 'baseline-b']
BASELINE_SHA = 'a4c20a8e4b5abb5ccb2e470ad5479b3387750155'
CANDIDATE_SHA = 'da6f9a3bf2a964a77c59f4a24abba93d46dce4b6'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def main():
    ...
    a, c, b = by['baseline-a'], by['vertex-attrib'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production vs interleaved vertex-attrib cache same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | ptr updates | ptr saved | enable changes | enable saved | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('attrib_pointer_updates','-')} | {r.get('attrib_pointer_saved','-')} | {r.get('attrib_enable_changes','-')} | {r.get('attrib_enable_saved','-')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted vertex-attrib delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('vertex-attrib', CANDIDATE_SHA)]:
        r = by[name]
    ...
    ptr_updates = int(c.get('attrib_pointer_updates') or 0)
    ptr_saved = int(c.get('attrib_pointer_saved') or 0)
