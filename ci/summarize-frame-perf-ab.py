...
ORDER = ['baseline-a', 'dev-noleak', 'baseline-b']
BASELINE_SHA = 'd4be7870e41eece34b9a73b3fe7a685818e4a5d6'
CANDIDATE_SHA = 'd864e05a5d9cd22c2add3873f1bb7330dbc0c1e9'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(ok=data.get('ok'), fatal=data.get('fatalSeenAt'), errors=len(data.get('errors') or []),
                   ...
                   webgl_draws=gp.get('webglDrawDelta'))
...
def main():
    ...
    a, c, b = by['baseline-a'], by['dev-noleak'], by['baseline-b']
    ...
    expected_shortcut = (float(a['shortcut_avg_ms']) + float(b['shortcut_avg_ms'])) / 2.0 if a.get('shortcut_avg_ms') is not None and b.get('shortcut_avg_ms') is not None else None
    ...
    shortcut_delta = None if expected_shortcut is None or c.get('shortcut_avg_ms') is None else float(c['shortcut_avg_ms']) - expected_shortcut
    c['drift_adjusted'] = {
        'fps_expected': expected_fps, 'fps_delta': fps_delta, 'fps_pct': None if expected_fps in (None,0) or fps_delta is None else fps_delta/expected_fps*100.0,
        'frame_expected': expected_frame, 'frame_delta': frame_delta, 'frame_pct': None if expected_frame in (None,0) or frame_delta is None else frame_delta/expected_frame*100.0,
        'shortcut_expected': expected_shortcut, 'shortcut_delta': shortcut_delta, 'shortcut_pct': None if expected_shortcut in (None,0) or shortcut_delta is None else shortcut_delta/expected_shortcut*100.0,
    }
    ROOT.mkdir(parents=True, exist_ok=True)
    (ROOT/'summary.json').write_text(json.dumps(rows, indent=2)+'\n', encoding='utf-8')
    lines=['# Production vs dev-mode no-leak same-runner A/B','',f'Fixed seed `{SEED}`; expected world `218/917/59/21`.','',
           '| variant | rc/verify | FPS | frame ms | shortcut avg | world |','|---|---:|---:|---:|---:|---|']
    for r in rows:
        world='-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {world} |")
    d=c['drift_adjusted']
    lines += ['', 'Drift-adjusted dev-no-leak delta:',
              '- fps=n/a' if d['fps_delta'] is None else f"- fps={d['fps_delta']:+.3f} ({d['fps_pct']:+.2f}%)",
              '- frame_ms=n/a' if d['frame_delta'] is None else f"- frame_ms={d['frame_delta']:+.3f} ({d['frame_pct']:+.2f}%)",
              '- shortcut_avg_ms=n/a' if d['shortcut_delta'] is None else f"- shortcut_avg_ms={d['shortcut_delta']:+.2f} ({d['shortcut_pct']:+.2f}%)"]
    (ROOT/'summary.md').write_text('\n'.join(lines)+'\n', encoding='utf-8')
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('dev-noleak', CANDIDATE_SHA)]:
        r=by[name]
