...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            lazy_color=bool(gp.get('lazyCurrentColorObserved')),
...
def main():
    ...
    lines = [
        '# Pointer-dirty production vs lazy current color same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | lazy color | draws | uploads | bytes | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('lazy_color',False)} | {r.get('interleaved_draws','-')} | {r.get('interleaved_uploads','-')} | {r.get('interleaved_bytes','-')} | {world} |")
    d = c['drift_adjusted']
    ...
    if c.get('lazy_color') is not True:
        raise SystemExit(f'lazy-current-color candidate did not activate: {c}')
    if a.get('lazy_color') or b.get('lazy_color'):
        raise SystemExit(f'production baseline unexpectedly reports lazy color activation: A={a} B={b}')
    return 0
