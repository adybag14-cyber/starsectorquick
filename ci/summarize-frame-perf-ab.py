...
ORDER = ['baseline-a', 'immediate-persistent-safe', 'baseline-b']
BASELINE_SHA = 'a4c20a8e4b5abb5ccb2e470ad5479b3387750155'
CANDIDATE_SHA = 'dbaef3a803defd9f228f79dd178146a94a9a5c03'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        ps = data.get('presentationStats') or {}
        row.update(
            ...
            persistent_growths=ps.get('immediateInterleavedBufferGrowths'),
        )
    ...
    if lp.exists():
        ...
        row['buffer_overflow_count'] = text.count('bufferSubData: buffer overflow')
    return row
...
def main():
    ...
    a, c, b = by['baseline-a'], by['immediate-persistent-safe'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production vs safe persistent immediate-buffer same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | draws | uploads | bytes | growths | overflows | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('interleaved_draws','-')} | {r.get('interleaved_uploads','-')} | {r.get('interleaved_bytes','-')} | {r.get('persistent_growths','-')} | {r.get('buffer_overflow_count','-')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted immediate-persistent-safe delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('immediate-persistent-safe', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if int(c.get('interleaved_draws') or 0) < 1000 or int(c.get('interleaved_uploads') or 0) < 1000 or int(c.get('persistent_growths') or 0) <= 0:
        raise SystemExit(f'persistent activation evidence invalid: {c}')
    if int(c.get('buffer_overflow_count') or 0) != 0:
        raise SystemExit(f'persistent upload overflow detected: {c}')
    return 0
