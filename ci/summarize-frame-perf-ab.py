...
ORDER = ['baseline-a', 'vertex-attrib-lean', 'baseline-b']
BASELINE_SHA = 'a4c20a8e4b5abb5ccb2e470ad5479b3387750155'
CANDIDATE_SHA = '8c17f91c503f06f70d178ea8bf4660c9a64756fe'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            attrib_enable_hit=bool(gp.get('vertexAttribEnableCacheHitObserved')),
        )
...
def main():
    ...
    a, c, b = by['baseline-a'], by['vertex-attrib-lean'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production vs lean interleaved vertex-attrib cache same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | ptr hit | enable hit | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('attrib_pointer_hit','-')} | {r.get('attrib_enable_hit','-')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted lean vertex-attrib delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('vertex-attrib-lean', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if c.get('attrib_pointer_hit') is not True or c.get('attrib_enable_hit') is not True:
        raise SystemExit(f'lean vertex-attrib cache activation evidence invalid: {c}')
    return 0
