...
ORDER = ['baseline-a', 'upload-view-cache', 'baseline-b']
BASELINE_SHA = '60e21b1fef5985f69e0de1fd39290df696a67033'
CANDIDATE_SHA = '3aab270cbf4ffb8e82f6a89b1e5a7b1329ee2581'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            upload_view_cache_active=bool(gp.get('immediateUploadViewCacheActive')),
...
def main():
    ...
    a, c, b = by['baseline-a'], by['upload-view-cache'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Current production vs immediate upload-view cache same-runner A/B', '',
    ...
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted immediate upload-view cache delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('upload-view-cache', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if int(c.get('interleaved_draws') or 0) < 1000 or int(c.get('interleaved_uploads') or 0) < 1000 or c.get('upload_view_cache_active') is not True:
        raise SystemExit(f'upload-view cache activation evidence invalid: {c}')
    if a.get('upload_view_cache_active') or b.get('upload_view_cache_active'):
        raise SystemExit(f'upload-view cache marker leaked into baseline: a={a} b={b}')
    return 0
