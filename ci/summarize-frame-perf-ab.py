...
ORDER = ['baseline-a', 'counter-trim', 'baseline-b']
BASELINE_SHA = '60e21b1fef5985f69e0de1fd39290df696a67033'
CANDIDATE_SHA = 'bd7b6dc1503f83cf3b8ffb88a71bb3af1a98f6c8'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            counter_trim_active=bool(gp.get('immediateCounterTrimActive')),
...
def main():
    ...
    a, c, b = by['baseline-a'], by['counter-trim'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Pointer-dirty production vs immediate counter-trim same-runner A/B', '',
    ...
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted counter-trim delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('counter-trim', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if int(c.get('interleaved_draws') or 0) < 1000 or int(c.get('interleaved_uploads') or 0) < 1000 or c.get('counter_trim_active') is not True:
        raise SystemExit(f'counter-trim activation evidence invalid: {c}')
    if a.get('counter_trim_active') or b.get('counter_trim_active'):
        raise SystemExit(f'counter-trim leaked into baseline: a={a} b={b}')
    return 0
