...
ORDER = ['baseline-a', 'draw-telemetry-off', 'baseline-b']
BASELINE_SHA = '60e21b1fef5985f69e0de1fd39290df696a67033'
CANDIDATE_SHA = '7e647d77af7f380ad1d6e137c2e008372c91db0e'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            draw_telemetry_present=bool(gp.get('detailedDrawTelemetryPresent')),
            draw_telemetry_active=bool(gp.get('detailedDrawTelemetryActive')),
...
def main():
    ...
    a, c, b = by['baseline-a'], by['draw-telemetry-off'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Current production vs detailed draw telemetry opt-out 240-frame same-runner A/B', '',
    ...
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted detailed draw telemetry opt-out delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('draw-telemetry-off', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if int(a.get('interleaved_draws') or 0) < 1000 or int(b.get('interleaved_draws') or 0) < 1000:
        raise SystemExit(f'baseline detailed draw telemetry unexpectedly inactive: a={a} b={b}')
    if int(a.get('interleaved_uploads') or 0) < 1000 or int(b.get('interleaved_uploads') or 0) < 1000:
        raise SystemExit(f'baseline upload telemetry unexpectedly inactive: a={a} b={b}')
    if int(c.get('interleaved_draws') or 0) != 0 or int(c.get('interleaved_uploads') or 0) != 0:
        raise SystemExit(f'candidate detailed draw telemetry did not stay off: {c}')
    if c.get('draw_telemetry_present') is not True or c.get('draw_telemetry_active') is not False:
        raise SystemExit(f'candidate telemetry opt-in marker invalid: {c}')
    if a.get('draw_telemetry_present') or b.get('draw_telemetry_present'):
        raise SystemExit(f'candidate telemetry marker leaked into baseline: a={a} b={b}')
    return 0
