...
ORDER = ['baseline-a', 'static-draw-telemetry', 'baseline-b']
BASELINE_SHA = 'bb67299ca71fe57fb10c083cb6ef3c4d460dfcdf'
CANDIDATE_SHA = 'edbd73937b39cd2d240013ac1a71f710903dc630'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            swaps=gp.get('swapDelta'), static_draw_telemetry_active=bool(gp.get('detailedDrawTelemetryStaticDispatchActive')),
        )
...
def main():
    ...
    a, c, b = by['baseline-a'], by['static-draw-telemetry'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production baseline vs static draw telemetry same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | swaps | static draw telemetry | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('swaps','-')} | {r.get('static_draw_telemetry_active')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted static draw telemetry delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
        lines.append(f"- {metric}=n/a" if v['delta'] is None else f"- {metric}={v['delta']:+.3f} ({v['pct']:+.2f}%)")
    (ROOT / 'summary.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('static-draw-telemetry', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if c.get('static_draw_telemetry_active') is not True:
        raise SystemExit(f'static draw telemetry activation missing: {c}')
    if a.get('static_draw_telemetry_active') or b.get('static_draw_telemetry_active'):
        raise SystemExit(f'static draw telemetry marker leaked into baseline: a={a} b={b}')
    return 0
