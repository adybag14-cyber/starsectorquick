...
ORDER = ['baseline-a', 'pointer-attrib', 'baseline-b']
BASELINE_SHA = 'a4c20a8e4b5abb5ccb2e470ad5479b3387750155'
CANDIDATE_SHA = '9bc861da346dca5a71f4da45d043e8db74e6e385'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            attrib_pointer_hit=bool(gp.get('vertexAttribPointerCacheHitObserved')),
...
def main():
    ...
    a, c, b = by['baseline-a'], by['pointer-attrib'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production vs interleaved pointer-only cache same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | ptr hit | ptr updates | ptr saved | enable changes | enable saved | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('attrib_pointer_hit','-')} | {r.get('attrib_pointer_updates','-')} | {r.get('attrib_pointer_saved','-')} | {r.get('attrib_enable_changes','-')} | {r.get('attrib_enable_saved','-')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted pointer-attrib delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('pointer-attrib', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if c.get('attrib_pointer_hit') is not True:
        raise SystemExit(f'pointer-only cache did not activate: {c}')
    if a.get('attrib_pointer_hit') or b.get('attrib_pointer_hit'):
        raise SystemExit(f'production baseline unexpectedly reports pointer-cache hit: A={a} B={b}')
    if int(c.get('interleaved_draws') or 0) < 1000 or int(c.get('interleaved_uploads') or 0) < 1000:
        raise SystemExit(f'pointer-only traffic evidence invalid: {c}')
    return 0
