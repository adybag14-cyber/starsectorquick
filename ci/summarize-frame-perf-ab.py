...
ORDER = ['baseline-a', 'quad-index', 'baseline-b']
BASELINE_SHA = 'a4c20a8e4b5abb5ccb2e470ad5479b3387750155'
CANDIDATE_SHA = '000b0088d06bcd3598cb266c373a031486f863bf'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            webgl_draws=gp.get('webglDrawDelta'), quad_batches=gp.get('quadBatchDelta'), quad_saved=gp.get('quadDrawCallsSavedDelta'),
...
def main():
    ...
    a, c, b = by['baseline-a'], by['quad-index'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production vs sticky quad-index bind same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | quad batches | quad draws saved | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('quad_batches','-')} | {r.get('quad_saved','-')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted sticky-quad-index delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('quad-index', CANDIDATE_SHA)]:
        r = by[name]
    ...
    quad_batches = int(c.get('quad_batches') or 0)
    quad_saved = int(c.get('quad_saved') or 0)
    if quad_batches < 100 or quad_saved <= 0 or int(c.get('webgl_draws') or 0) <= 0:
        raise SystemExit(f'quad-index cache evidence invalid: {c}')
    return 0
