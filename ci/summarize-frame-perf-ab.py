...
ORDER = ['baseline-a', 'core-write-skip', 'baseline-b']
BASELINE_SHA = 'a4c20a8e4b5abb5ccb2e470ad5479b3387750155'
CANDIDATE_SHA = '9ba30dc1fe67a26a31389f6ec8a6d3b08460f11f'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def main():
    ...
    a, c, b = by['baseline-a'], by['core-write-skip'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted core-write-skip delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('core-write-skip', CANDIDATE_SHA)]:
        r = by[name]
    ...
    calls = int(c.get('core_calls') or 0)
    skipped = int(c.get('core_skipped') or 0)
    changes = int(c.get('core_changes') or 0)
    avoided = int(c.get('core_queries_avoided') or 0)
    if calls < 100 or skipped < 100 or changes <= 0 or skipped + changes != calls:
        raise SystemExit(f'core-write-skip cache activation invalid: {c}')
    if avoided != 0:
        raise SystemExit(f'core-write-skip must preserve snapshot queries: {c}')
    return 0
