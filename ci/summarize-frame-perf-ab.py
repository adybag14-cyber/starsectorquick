...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(ok=data.get('ok'), fatal=data.get('fatalSeenAt'), errors=len(data.get('errors') or []),
          ...
          core_state_skipped=gp.get('coreStateSkippedDelta'), core_queries_avoided=gp.get('coreStateSnapshotQueriesAvoidedDelta'),
          color_array_saved=gp.get('colorAttribArrayCallsSavedDelta'), color_uploads_saved=gp.get('colorAttribConstantUploadsSavedDelta'))
...
def main():
    ...
    for name in ['dev-mode','pacer','leak-off']:
        ...
        lines.append(f"  counters: texture={r.get('texture_bind_calls','-')}calls/{r.get('texture_bind_changes','-')}changes/{r.get('texture_bind_skipped','-')}skipped core={r.get('core_state_skipped','-')} skipped/{r.get('core_queries_avoided','-')} queriesAvoided color={r.get('color_array_saved','-')} arraySaved/{r.get('color_uploads_saved','-')} valueSaved matrixInPlace={r.get('matrix_inplace_ops','-')} ops/{r.get('matrix_temp_alloc_avoided','-')} tempAllocAvoided")
