...
ORDER = ['baseline-a', 'mouse-int-pair', 'baseline-b']
BASELINE_SHA = 'a4c20a8e4b5abb5ccb2e470ad5479b3387750155'
CANDIDATE_SHA = '62ed1bde14177672586a1ab94bb0ebe91d960dac'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row['mouse_meta_snapshots'] = int(input_stats.get('mousePollMetaSnapshots') or 0)
...
def main():
    ...
    a, c, b = by['baseline-a'], by['mouse-int-pair'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production vs mouse int-pair snapshot same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | pos snaps | meta snaps | mouse pos JNI | mouse button JNI | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('mouse_snapshots','-')} | {r.get('mouse_meta_snapshots','-')} | {r.get('mouse_position_queries','-')} | {r.get('mouse_button_queries','-')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted mouse-int-pair delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('mouse-int-pair', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if c.get('mouse_meta_snapshots', 0) < 100 or abs(c.get('mouse_meta_snapshots', 0) - c.get('mouse_snapshots', 0)) > 1:
        raise SystemExit(f'mouse int-pair meta snapshot marker inactive/mismatched: {c}')
