...
ORDER = ['baseline-a', 'stable-attrib', 'baseline-b']
BASELINE_SHA = 'bb67299ca71fe57fb10c083cb6ef3c4d460dfcdf'
CANDIDATE_SHA = 'b2bbcbc37c8391388dd1f4ddf91bb2389442a703'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            swaps=gp.get('swapDelta'), stable_attrib_active=bool(gp.get('immediateStableAttribEnableActive')),
        )
...
def main():
    ...
    a, c, b = by['baseline-a'], by['stable-attrib'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Current production vs stable immediate attrib-enable 240-frame same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | swaps | stable attrib | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('swaps','-')} | {r.get('stable_attrib_active')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted stable immediate attrib-enable delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
        lines.append(f"- {metric}=n/a" if v['delta'] is None else f"- {metric}={v['delta']:+.3f} ({v['pct']:+.2f}%)")
    (ROOT / 'summary.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('stable-attrib', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if c.get('stable_attrib_active') is not True:
        raise SystemExit(f'stable attrib-enable activation missing: {c}')
    if a.get('stable_attrib_active') or b.get('stable_attrib_active'):
        raise SystemExit(f'stable attrib-enable marker leaked into baseline: a={a} b={b}')
    return 0
