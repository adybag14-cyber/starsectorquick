...
ORDER = ['baseline-a', 'color-defer-attrib-enable-dirty', 'baseline-b']
BASELINE_SHA = '0eedc59892cb839fc747e1a31eac295198896297'
CANDIDATE_SHA = '3a0e4c47a8382a65547e4ab0ee565115fb000574'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def main():
    ...
    a, c, b = by['baseline-a'], by['color-defer-attrib-enable-dirty'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Color-defer production vs attrib-enable-dirty stack same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | attrib refreshes | draws | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('attrib_refreshes','-')} | {r.get('interleaved_draws','-')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted color-defer plus attrib-enable-dirty delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('color-defer-attrib-enable-dirty', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if draws < 1000 or int(c.get('interleaved_uploads') or 0) < 1000:
        raise SystemExit(f'color-defer-attrib-enable-dirty traffic evidence invalid: {c}')
    if refreshes <= 0 or refreshes >= max(2, draws // 10):
        raise SystemExit(f'color-defer-attrib-enable-dirty refresh evidence invalid: refreshes={refreshes} draws={draws} row={c}')
