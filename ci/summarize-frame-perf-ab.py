...
ORDER = ['baseline-a', 'playtesting-mode', 'baseline-b']
BASELINE_SHA = 'bb67299ca71fe57fb10c083cb6ef3c4d460dfcdf'
CANDIDATE_SHA = '32ab3a5e0b781547c901119d7e4c30ebb0cd9817'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            swaps=gp.get('swapDelta'), served_playtesting_mode=bool(data.get('servedPlaytestingMode')),
        )
...
def main():
    ...
    a, c, b = by['baseline-a'], by['playtesting-mode'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production baseline vs playtesting-mode same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | swaps | playtesting | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('swaps','-')} | {r.get('served_playtesting_mode')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted playtesting-mode delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
        lines.append(f"- {metric}=n/a" if v['delta'] is None else f"- {metric}={v['delta']:+.3f} ({v['pct']:+.2f}%)")
    (ROOT / 'summary.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('playtesting-mode', CANDIDATE_SHA)]:
        r = by[name]
    ...
    if c.get('served_playtesting_mode') is not True:
        raise SystemExit(f'playtesting mode activation missing: {c}')
    if a.get('served_playtesting_mode') or b.get('served_playtesting_mode'):
        raise SystemExit(f'playtesting mode leaked into baseline: a={a} b={b}')
    return 0
