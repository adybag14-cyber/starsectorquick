...
ORDER = ['baseline-a', 'compact-color', 'baseline-b']
BASELINE_SHA = 'a4c20a8e4b5abb5ccb2e470ad5479b3387750155'
CANDIDATE_SHA = 'b536cc5b208781fa18c3b88c5b2428b002347fcc'
SEED = 'SEK968276040'
WORLD = (218, 917, 59, 21)
...
def parse(name):
    ...
    if rp.exists():
        ...
        row.update(
            ...
            compact_draws=gp.get('immediateCompactColorDrawDelta'), compact_vertices=gp.get('immediateCompactColorVertexDelta'),
            compact_saved_bytes=gp.get('immediateCompactColorBytesSavedDelta'), compact_fallback=gp.get('immediateCompactColorFallbackDrawDelta'),
            compact_backfill=gp.get('immediateCompactColorBackfillVertexDelta'),
        )
...
def main():
    ...
    a, c, b = by['baseline-a'], by['compact-color'], by['baseline-b']
    c['drift_adjusted'] = {}
    ...
    lines = [
        '# Production vs compact-color immediate layout same-runner A/B', '',
        f'Fixed seed `{SEED}`; expected world `218/917/59/21`.', '',
        '| variant | rc/verify | FPS | frame ms | p95 | p99 | jitter p95 | shortcut avg | upload bytes | compact draws | saved bytes | fallback | world |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|',
    ]
    for r in rows:
        world = '-' if not r.get('world') else '/'.join(str(x) for x in r['world'])
        lines.append(f"| {r['name']} | {r.get('rc','-')} / {r.get('verify_rc','-')} | {fmt(r.get('fps'))} | {fmt(r.get('frame_ms'))} | {fmt(r.get('p95_ms'))} | {fmt(r.get('p99_ms'))} | {fmt(r.get('jitter_p95_ms'))} | {fmt(r.get('shortcut_avg_ms'))} | {r.get('interleaved_bytes','-')} | {r.get('compact_draws','-')} | {r.get('compact_saved_bytes','-')} | {r.get('compact_fallback','-')} | {world} |")
    d = c['drift_adjusted']
    lines += ['', 'Drift-adjusted compact-color delta:']
    for metric in ('fps','frame_ms','p95_ms','p99_ms','jitter_p95_ms'):
        v = d[metric]
    ...
    print('\n'.join(lines))
    for name, expected_ref in [('baseline-a', BASELINE_SHA), ('baseline-b', BASELINE_SHA), ('compact-color', CANDIDATE_SHA)]:
        r = by[name]
    ...
    compact_draws = int(c.get('compact_draws') or 0)
    compact_vertices = int(c.get('compact_vertices') or 0)
    saved_bytes = int(c.get('compact_saved_bytes') or 0)
    if compact_draws < 100 or compact_vertices <= 0 or saved_bytes <= 0:
        raise SystemExit(f'compact-color evidence invalid: {c}')
    return 0
