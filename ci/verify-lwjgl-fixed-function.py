...
def main() -> int:
    ...
    require(text, "LWJGL_MATRIX_STACK_POOL_V1", "module")
    ...
    get_matrix_top = function_block(text, "getCurMatrixTop")
    set_matrix_top = function_block(text, "setCurMatrixTop")
    ...
    acquire_matrix = function_block(text, "acquireCompatMatrixStackEntry")
    release_matrix = function_block(text, "releaseCompatMatrixStackEntry")
    ...
    require(tex_image, "textureStorageWidth[boundTexture2DId] = width;", "texture storage width tracking")
    require(tex_image, "textureStorageHeight[boundTexture2DId] = height;", "texture storage height tracking")
    require(tex_sub_image, "glCtx.generateMipmap(target);", "level-0 texture sub-upload mipmaps")
    require(tex_sub_image, "var storageFormat = trackedStorageFormat || (format == glCtx.RGB ? glCtx.RGBA : format);", "texture sub-upload storage compatibility")
    ...
    require(tex_sub_image, "LWJGL_TEX_SUBIMAGE_STORAGE_COMPAT_V1", "undefined texture storage compatibility")
    require(tex_sub_image, "glCtx.texImage2D(target, level, upload.format", "undefined level-zero texture promotion")
    require(tex_sub_image, "regionExceedsStorage", "undersized placeholder growth detection")
    ...
    require(draw_arrays, "if(modelViewMatrixUniformDirty)", "model-view matrix dirty upload")
    require(draw_arrays, "if(projMatrixUniformDirty)", "projection matrix dirty upload")
    require(draw_arrays, "if(textureMatrixUniformDirty)", "texture matrix dirty upload")
    require(draw_arrays, "glCtx.uniformMatrix4fv(texMatrixLocation, false, textureMatrixStack[textureMatrixStack.length - 1]);", "texture matrix upload")
    require(get_matrix_top, "ensureCurMatrixStack();", "matrix getter stack guard")
    require(set_matrix_top, "ensureCurMatrixStack();", "matrix setter stack guard")
    require(set_matrix_top, "markCurMatrixUniformDirty();", "matrix replacement invalidation")
    require(load_identity, "markCurMatrixUniformDirty();", "identity matrix invalidation")
    require(push_matrix, "ensureCurMatrixStack();", "matrix push stack guard")
    require(push_matrix, "acquireCompatMatrixStackEntry", "matrix push pooling")
    require(pop_matrix, "if(stack.length <= 1)", "matrix pop underflow guard")
    require(pop_matrix, "releaseCompatMatrixStackEntry(stack.pop());", "matrix pop pooling")
    require(pop_matrix, "markCurMatrixUniformDirty();", "matrix pop invalidation")
    require(acquire_matrix, "compatMatrixStackPool.pop()", "matrix pool reuse")
    require(acquire_matrix, "glMatrix.mat4.create()", "matrix pool allocation fallback")
    require(acquire_matrix, "glMatrix.mat4.copy(out, source);", "matrix pool source copy")
    require(release_matrix, "compatMatrixStackPool.length < compatMatrixStackPoolMax", "bounded matrix pool release")
