...
expect(c.immediateAttribEnableDirty===false,'in-begin deferred color dirtied enable layout');
...
expect(c.immediateAttribEnableDirty===true,'outside-begin color fallback did not dirty enable layout');
expect(calls[1].slice(2).join(',')==='0.5,0.6,0.7,1','outside-begin generic color wrong');
c.immediateInterleavedEnabled=false;c.Java_org_lwjgl_opengl_GL11_nglBegin(null,7,0);calls.length=0;c.Java_org_lwjgl_opengl_GL11_nglColor4f(null,.9,.8,.7,.6,0);
expect(calls.length===2,'legacy immediate fallback must retain generic color updates');
console.log('verify-lwjgl-immediate-color-attrib-defer: OK deferred=true fallback=true');
