...
javac -cp .ci-build/asm/asm.jar -d .ci-build/transform \
  ...
  ci/PatchXStreamConvertTiming.java \
  ci/VerifyXStreamConvertTimingPatch.java \
...
jar tf jars/fixer_patch.jar | grep -qx 'com/fs/starfarer/BrowserXStreamConvertDiag.class'
java -cp .ci-build/asm/asm.jar:.ci-build/transform \
  PatchXStreamConvertTiming jars/xstream-1.4.10.jar .ci-build/xstream-convert-timing.jar
java -cp .ci-build/asm/asm.jar:.ci-build/transform \
  VerifyXStreamConvertTimingPatch .ci-build/xstream-convert-timing.jar
mv .ci-build/xstream-convert-timing.jar jars/xstream-1.4.10.jar
javap -classpath "jars/fixer_patch.jar:jars/xstream-1.4.10.jar:$CP" -c -p \
  com.thoughtworks.xstream.core.TreeUnmarshaller | grep -q 'BrowserXStreamConvertDiag.enter'
javap -classpath "jars/fixer_patch.jar:jars/xstream-1.4.10.jar:$CP" -c -p \
  com.thoughtworks.xstream.core.TreeUnmarshaller | grep -q 'BrowserXStreamConvertDiag.exit'
