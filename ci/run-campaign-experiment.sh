...
javac -cp .ci-build/asm/asm.jar -d .ci-build/transform \
  ...
  ci/PatchShipSlotCoverNullDiagnostics.java
...
if [[ "${STARSECTOR_SLOT_COVER_DIAGNOSTICS:-false}" == "true" ]]; then
  java -cp .ci-build/asm/asm.jar:.ci-build/transform \
    PatchShipSlotCoverNullDiagnostics jars/starfarer_obf.jar .ci-build/starfarer-slot-cover-null-diag.jar
  mv .ci-build/starfarer-slot-cover-null-diag.jar jars/starfarer_obf.jar
  javap -classpath jars/starfarer_obf.jar -c -p com.fs.starfarer.combat.entities.Ship \
    > "$OUT/ship-slot-cover-diagnostic.javap"
  grep -q 'ShipSlotCoverNullDiag:' "$OUT/ship-slot-cover-diagnostic.javap"
fi
