...
rm -rf .ci-build/verify-browser-chunked-base64
mkdir -p .ci-build/verify-browser-chunked-base64
javac -encoding UTF-8 --release 8 -cp "jars/fixer_patch.jar:$CP" \
  -d .ci-build/verify-browser-chunked-base64 ci/VerifyBrowserChunkedBase64Transport.java
java -Xverify:all -cp ".ci-build/verify-browser-chunked-base64:jars/fixer_patch.jar:$CP" \
  VerifyBrowserChunkedBase64Transport
...
javac -cp .ci-build/asm/asm.jar -d .ci-build/transform \
  ...
  ci/PatchWeaponDiscoveryDiagnostics.java \
  ci/VerifyWeaponDiscoveryDiagnosticsPatch.java \
...
# Diagnostic-only timing of weapon/projectile source enumeration versus parsing.
java -cp .ci-build/asm/asm.jar:.ci-build/transform \
  PatchWeaponDiscoveryDiagnostics jars/starfarer_obf.jar .ci-build/starfarer-weapon-discovery-diag.jar
java -cp .ci-build/asm/asm.jar:.ci-build/transform \
  VerifyWeaponDiscoveryDiagnosticsPatch .ci-build/starfarer-weapon-discovery-diag.jar jars/fixer_patch.jar
mv .ci-build/starfarer-weapon-discovery-diag.jar jars/starfarer_obf.jar
java -cp .ci-build/asm/asm.jar:.ci-build/transform \
  PatchWeaponDiscoveryDiagnostics jars/starfarer_obf.jar .ci-build/starfarer-weapon-discovery-diag-repeat.jar
java -cp .ci-build/asm/asm.jar:.ci-build/transform \
  VerifyWeaponDiscoveryDiagnosticsPatch .ci-build/starfarer-weapon-discovery-diag-repeat.jar jars/fixer_patch.jar
cmp -s jars/starfarer_obf.jar .ci-build/starfarer-weapon-discovery-diag-repeat.jar
...
java -Xverify:all -cp ".ci-build/verify:jars/fixer_patch.jar:$CP" \
  ...
  com.fs.starfarer.loading.BrowserWeaponDiscoveryDiag \
...
grep -q 'BrowserWeaponDiscoveryDiag: kind=projectiles' "$OUT/browser.log"
grep -q 'BrowserWeaponDiscoveryDiag: kind=weapons' "$OUT/browser.log"
