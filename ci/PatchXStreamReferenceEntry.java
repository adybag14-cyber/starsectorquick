...
/** Diagnostic-only entry hook around AbstractReferenceUnmarshaller.convert(). */
public final class PatchXStreamReferenceEntry {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) throw new IllegalArgumentException("usage: PatchXStreamReferenceEntry input.jar output.jar");
        ...
        try (JarFile jar = new JarFile(input.toFile());
            ...
            while (entries.hasMoreElements()) {
                ...
                if (TARGET.equals(entry.getName())) {
                    classes[0]++;
                    bytes = patch(bytes, methods, enters);
                }
                out.write(bytes);
                out.closeEntry();
            }
        }
        if (classes[0] != 1 || methods[0] != 1 || enters[0] != 1) {
            Files.deleteIfExists(output);
            throw new IllegalStateException("XStream reference entry shape mismatch classes=" + classes[0]
                    + " methods=" + methods[0] + " enters=" + enters[0]);
        }
        System.out.println("Patched XStream reference entry methods=1 enters=1");
    }

    private static byte[] patch(byte[] input, int[] methods, int[] enters) {
