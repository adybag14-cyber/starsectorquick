...
public final class PatchResourceQueueProfile {
    ...
    public static void main(String[] args)throws Exception{
        ...
        Path input=Path.of(args[0]),output=Path.of(args[1]); int[] classes={0},queueMethods={0},initMethods={0},record={0},queueSummary={0},begin={0},end={0},loadSummary={0};
        try(JarFile jar=new JarFile(input.toFile());JarOutputStream out=new JarOutputStream(Files.newOutputStream(output))){
            Enumeration<JarEntry> es=jar.entries();while(es.hasMoreElements()){JarEntry e=es.nextElement();JarEntry c=new JarEntry(e.getName());c.setTime(e.getTime());out.putNextEntry(c);byte[] bytes;try(InputStream in=jar.getInputStream(e)){bytes=readAll(in);}if(ENTRY.equals(e.getName())){classes[0]++;bytes=patch(bytes,queueMethods,initMethods,record,queueSummary,begin,end,loadSummary);}out.write(bytes);out.closeEntry();}
        }
        if(classes[0]!=1||queueMethods[0]!=1||initMethods[0]!=1||record[0]!=1||queueSummary[0]!=1||begin[0]!=1||end[0]!=2||loadSummary[0]!=1){Files.deleteIfExists(output);throw new IllegalStateException("queue profile mismatch classes="+classes[0]+" queueMethods="+queueMethods[0]+" initMethods="+initMethods[0]+" record="+record[0]+" queueSummary="+queueSummary[0]+" begin="+begin[0]+" end="+end[0]+" loadSummary="+loadSummary[0]);}
        System.out.println("Patched ResourceLoader queue profiler record=1 queueSummary=1 begin=1 end=2 loadSummary=1");
    }

    private static byte[] patch(byte[] input,int[] queueMethods,int[] initMethods,int[] record,int[] queueSummary,int[] begin,int[] end,int[] loadSummary){
        int[] existing={0,0,0,0,0};
        new ClassReader(input).accept(new ClassVisitor(Opcodes.ASM9){@Override public MethodVisitor visitMethod(int a,String n,String d,String s,String[] ex){return new MethodVisitor(Opcodes.ASM9){@Override public void visitMethodInsn(int op,String owner,String name,String desc,boolean itf){if(op==Opcodes.INVOKESTATIC&&HELPER.equals(owner)){if("record".equals(name))existing[0]++;else if("printQueueSummary".equals(name))existing[1]++;else if("beginLoad".equals(name))existing[2]++;else if("endLoad".equals(name))existing[3]++;else if("printLoadSummary".equals(name))existing[4]++;}}};}},0);
        if(existing[0]==1&&existing[1]==1&&existing[2]==1&&existing[3]==2&&existing[4]==1){queueMethods[0]=1;initMethods[0]=1;record[0]=1;queueSummary[0]=1;begin[0]=1;end[0]=2;loadSummary[0]=1;return input;}
