...
${fallback}`);
  ...
  if (deepGameplay && expectedState === 'campaign' && !fatalSeenAt) {
    ...
    gameplayPerformance = {
      ...
      colorAttribArrayChangesDelta: Number(perfAfter.colorAttribArrayChanges || 0) - Number(perfBefore.colorAttribArrayChanges || 0),
      colorAttribArrayCallsSavedDelta: Number(perfAfter.colorAttribArrayCallsSaved || 0) - Number(perfBefore.colorAttribArrayCallsSaved || 0),
      colorAttribConstantUploadsDelta: Number(perfAfter.colorAttribConstantUploads || 0) - Number(perfBefore.colorAttribConstantUploads || 0),
      colorAttribConstantUploadsSavedDelta: Number(perfAfter.colorAttribConstantUploadsSaved || 0) - Number(perfBefore.colorAttribConstantUploadsSaved || 0),
    };
    gameplayPerformance.responsive = gameplayPerformance.swapDelta >= 20 && gameplayPerformance.recentFps >= 2;
    logs.push(`[gameplay-performance] durationMs=${gameplayPerformance.durationMs} swaps=${gameplayPerformance.swapDelta} fps=${gameplayPerformance.recentFps.toFixed(2)} frameMs=${gameplayPerformance.recentFrameMs.toFixed(2)} p50=${gameplayPerformance.frameP50Ms.toFixed(2)} p95=${gameplayPerformance.frameP95Ms.toFixed(2)} p99=${gameplayPerformance.frameP99Ms.toFixed(2)} max=${gameplayPerformance.frameMaxMs.toFixed(2)} jitterStdDev=${gameplayPerformance.frameJitterStdDevMs.toFixed(2)} jitterP95=${gameplayPerformance.frameJitterP95Ms.toFixed(2)} longFrames=${gameplayPerformance.longFrameDelta} droppedEstimate=${gameplayPerformance.droppedFrameEstimateDelta} webglDraws=${gameplayPerformance.webglDrawDelta} quadBatches=${gameplayPerformance.quadBatchDelta} quads=${gameplayPerformance.quadCountDelta} drawCallsSaved=${gameplayPerformance.quadDrawCallsSavedDelta} colorArrayChanges=${gameplayPerformance.colorAttribArrayChangesDelta} colorArraySaved=${gameplayPerformance.colorAttribArrayCallsSavedDelta} colorUploads=${gameplayPerformance.colorAttribConstantUploadsDelta} colorUploadsSaved=${gameplayPerformance.colorAttribConstantUploadsSavedDelta} responsive=${gameplayPerformance.responsive}`);
  }
