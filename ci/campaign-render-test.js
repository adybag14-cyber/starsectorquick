...
(async () => {
  ...
  if (deepGameplay && expectedState === 'campaign' && !fatalSeenAt) {
    ...
    gameplayPerformance = {
      ...
      immediateUniformColorDrawDelta: Number(perfAfter.immediateUniformColorDraws || 0) - Number(perfBefore.immediateUniformColorDraws || 0),
      immediateVaryingColorDrawDelta: Number(perfAfter.immediateVaryingColorDraws || 0) - Number(perfBefore.immediateVaryingColorDraws || 0),
      immediateUniformColorVertexDelta: Number(perfAfter.immediateUniformColorVertices || 0) - Number(perfBefore.immediateUniformColorVertices || 0),
      immediateVaryingColorVertexDelta: Number(perfAfter.immediateVaryingColorVertices || 0) - Number(perfBefore.immediateVaryingColorVertices || 0),
    };
    gameplayPerformance.responsive = gameplayPerformance.swapDelta >= 20 && gameplayPerformance.recentFps >= 2;
    logs.push(`[gameplay-performance] durationMs=${gameplayPerformance.durationMs} swaps=${gameplayPerformance.swapDelta} fps=${gameplayPerformance.recentFps.toFixed(2)} frameMs=${gameplayPerformance.recentFrameMs.toFixed(2)} webglDraws=${gameplayPerformance.webglDrawDelta} quadBatches=${gameplayPerformance.quadBatchDelta} quads=${gameplayPerformance.quadCountDelta} drawCallsSaved=${gameplayPerformance.quadDrawCallsSavedDelta} interleavedDraws=${gameplayPerformance.immediateInterleavedDrawDelta} interleavedUploads=${gameplayPerformance.immediateInterleavedUploadDelta} interleavedSaved=${gameplayPerformance.immediateInterleavedUploadsSavedDelta} interleavedBytes=${gameplayPerformance.immediateInterleavedBytesDelta} colorShape=uniform:${gameplayPerformance.immediateUniformColorDrawDelta}/${gameplayPerformance.immediateUniformColorVertexDelta},varying:${gameplayPerformance.immediateVaryingColorDrawDelta}/${gameplayPerformance.immediateVaryingColorVertexDelta} responsive=${gameplayPerformance.responsive}`);
  }
