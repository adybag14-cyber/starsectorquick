...
${fallback}`);
  ...
  if (deepGameplay && expectedState === 'campaign' && !fatalSeenAt) {
    ...
    gameplayPerformance = {
      ...
      clientArrayBufferAllocationsDelta: Number(perfAfter.clientArrayBufferAllocations || 0) - Number(perfBefore.clientArrayBufferAllocations || 0),
      clientArrayBufferSubDataDelta: Number(perfAfter.clientArrayBufferSubDataCalls || 0) - Number(perfBefore.clientArrayBufferSubDataCalls || 0),
      clientArrayBufferReuseDelta: Number(perfAfter.clientArrayBufferReuses || 0) - Number(perfBefore.clientArrayBufferReuses || 0),
      clientArrayLegacyBufferDataDelta: Number(perfAfter.clientArrayLegacyBufferDataCalls || 0) - Number(perfBefore.clientArrayLegacyBufferDataCalls || 0),
      clientArrayUploadBytesDelta: Number(perfAfter.clientArrayUploadBytes || 0) - Number(perfBefore.clientArrayUploadBytes || 0),
      clientArrayBufferCapacityBytes: Number(perfAfter.clientArrayBufferCapacityBytes || 0),
    };
    gameplayPerformance.responsive = gameplayPerformance.swapDelta >= 20 && gameplayPerformance.recentFps >= 2;
    logs.push(`[gameplay-performance] durationMs=${gameplayPerformance.durationMs} swaps=${gameplayPerformance.swapDelta} fps=${gameplayPerformance.recentFps.toFixed(2)} frameMs=${gameplayPerformance.recentFrameMs.toFixed(2)} p50=${gameplayPerformance.frameP50Ms.toFixed(2)} p95=${gameplayPerformance.frameP95Ms.toFixed(2)} p99=${gameplayPerformance.frameP99Ms.toFixed(2)} max=${gameplayPerformance.frameMaxMs.toFixed(2)} jitterStdDev=${gameplayPerformance.frameJitterStdDevMs.toFixed(2)} jitterP95=${gameplayPerformance.frameJitterP95Ms.toFixed(2)} longFrames=${gameplayPerformance.longFrameDelta} droppedEstimate=${gameplayPerformance.droppedFrameEstimateDelta} webglDraws=${gameplayPerformance.webglDrawDelta} quadBatches=${gameplayPerformance.quadBatchDelta} quads=${gameplayPerformance.quadCountDelta} drawCallsSaved=${gameplayPerformance.quadDrawCallsSavedDelta} clientAllocs=${gameplayPerformance.clientArrayBufferAllocationsDelta} clientSubData=${gameplayPerformance.clientArrayBufferSubDataDelta} clientReuses=${gameplayPerformance.clientArrayBufferReuseDelta} clientLegacyBufferData=${gameplayPerformance.clientArrayLegacyBufferDataDelta} clientUploadBytes=${gameplayPerformance.clientArrayUploadBytesDelta} clientCapacityBytes=${gameplayPerformance.clientArrayBufferCapacityBytes} responsive=${gameplayPerformance.responsive}`);
  }
