...
${fallback}`);
  ...
  if (deepGameplay && expectedState === 'campaign' && !fatalSeenAt) {
    ...
    gameplayPerformance = {
      ...
      clientRangeUploadCallsDelta: Number(perfAfter.clientRangeUploadCalls || 0) - Number(perfBefore.clientRangeUploadCalls || 0),
      clientRangeUploadBytesDelta: Number(perfAfter.clientRangeUploadBytes || 0) - Number(perfBefore.clientRangeUploadBytes || 0),
      clientRangeViewsAvoidedDelta: Number(perfAfter.clientRangeViewsAvoided || 0) - Number(perfBefore.clientRangeViewsAvoided || 0),
      clientRangeFallbackViewsDelta: Number(perfAfter.clientRangeFallbackViews || 0) - Number(perfBefore.clientRangeFallbackViews || 0),
      clientMemoryViewRefreshesDelta: Number(perfAfter.clientMemoryViewRefreshes || 0) - Number(perfBefore.clientMemoryViewRefreshes || 0),
    };
    gameplayPerformance.responsive = gameplayPerformance.swapDelta >= 20 && gameplayPerformance.recentFps >= 2;
    logs.push(`[gameplay-performance] durationMs=${gameplayPerformance.durationMs} swaps=${gameplayPerformance.swapDelta} fps=${gameplayPerformance.recentFps.toFixed(2)} frameMs=${gameplayPerformance.recentFrameMs.toFixed(2)} p50=${gameplayPerformance.frameP50Ms.toFixed(2)} p95=${gameplayPerformance.frameP95Ms.toFixed(2)} p99=${gameplayPerformance.frameP99Ms.toFixed(2)} max=${gameplayPerformance.frameMaxMs.toFixed(2)} jitterStdDev=${gameplayPerformance.frameJitterStdDevMs.toFixed(2)} jitterP95=${gameplayPerformance.frameJitterP95Ms.toFixed(2)} longFrames=${gameplayPerformance.longFrameDelta} droppedEstimate=${gameplayPerformance.droppedFrameEstimateDelta} webglDraws=${gameplayPerformance.webglDrawDelta} quadBatches=${gameplayPerformance.quadBatchDelta} quads=${gameplayPerformance.quadCountDelta} drawCallsSaved=${gameplayPerformance.quadDrawCallsSavedDelta} rangeUploads=${gameplayPerformance.clientRangeUploadCallsDelta} rangeBytes=${gameplayPerformance.clientRangeUploadBytesDelta} rangeViewsAvoided=${gameplayPerformance.clientRangeViewsAvoidedDelta} rangeFallbackViews=${gameplayPerformance.clientRangeFallbackViewsDelta} memoryViewRefreshes=${gameplayPerformance.clientMemoryViewRefreshesDelta} responsive=${gameplayPerformance.responsive}`);
  }
