...
${fallback}`);
  ...
  if (deepGameplay && expectedState === 'campaign' && !fatalSeenAt) {
    ...
    gameplayPerformance = {
      ...
      textureBindCallsDelta: Number(perfAfter.textureBindCalls || 0) - Number(perfBefore.textureBindCalls || 0),
      textureBindChangesDelta: Number(perfAfter.textureBindChanges || 0) - Number(perfBefore.textureBindChanges || 0),
      textureBindSkippedDelta: Number(perfAfter.textureBindSkipped || 0) - Number(perfBefore.textureBindSkipped || 0),
      textureBindInvalidationsDelta: Number(perfAfter.textureBindInvalidations || 0) - Number(perfBefore.textureBindInvalidations || 0),
    };
    gameplayPerformance.responsive = gameplayPerformance.swapDelta >= 20 && gameplayPerformance.recentFps >= 2;
    logs.push(`[gameplay-performance] durationMs=${gameplayPerformance.durationMs} swaps=${gameplayPerformance.swapDelta} fps=${gameplayPerformance.recentFps.toFixed(2)} frameMs=${gameplayPerformance.recentFrameMs.toFixed(2)} p50=${gameplayPerformance.frameP50Ms.toFixed(2)} p95=${gameplayPerformance.frameP95Ms.toFixed(2)} p99=${gameplayPerformance.frameP99Ms.toFixed(2)} max=${gameplayPerformance.frameMaxMs.toFixed(2)} jitterStdDev=${gameplayPerformance.frameJitterStdDevMs.toFixed(2)} jitterP95=${gameplayPerformance.frameJitterP95Ms.toFixed(2)} longFrames=${gameplayPerformance.longFrameDelta} droppedEstimate=${gameplayPerformance.droppedFrameEstimateDelta} webglDraws=${gameplayPerformance.webglDrawDelta} quadBatches=${gameplayPerformance.quadBatchDelta} quads=${gameplayPerformance.quadCountDelta} drawCallsSaved=${gameplayPerformance.quadDrawCallsSavedDelta} textureBinds=${gameplayPerformance.textureBindCallsDelta} textureChanges=${gameplayPerformance.textureBindChangesDelta} textureSkipped=${gameplayPerformance.textureBindSkippedDelta} textureInvalidations=${gameplayPerformance.textureBindInvalidationsDelta} responsive=${gameplayPerformance.responsive}`);
  }
