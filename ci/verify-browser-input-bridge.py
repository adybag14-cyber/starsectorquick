...
def main() -> int:
    ...
    for token in (
        ...
        reject(mouse, token, "Mouse per-query JNI bridge")

    require(js, "LWJGL_DIRECT_INPUT_BRIDGE_V1", "JavaScript input bridge")
    require(js, "LWJGL_MOUSE_POLL_SNAPSHOT_V1", "mouse poll snapshot")
    require(js, "inputStats.mousePollSnapshots++", "mouse poll snapshot stats")
    ...
    for token in (
        ...
        reject(js, token, "obsolete per-query mouse JNI exports")
