...
const attribDirtyPresent=src.includes('LWJGL_IMMEDIATE_ATTRIB_DIRTY_V1');
...
function make(enabled){
  ...
  const c=vm.createContext({
    ...
    presentationStats:{immediateInterleavedDraws:0,immediateInterleavedUploads:0,immediateInterleavedUploadsSaved:0,immediateInterleavedBytes:0,immediateAttribLayoutRefreshes:0},
...
if(attribDirtyPresent){
  expect(x.calls.filter(a=>a[0]==='pointer').length===3,'second immediate draw should reuse fixed pointer layout');
  expect(x.calls.filter(a=>a[0]==='enable').length===3,'second immediate draw should reuse enabled attribute layout');
  expect(c.presentationStats.immediateAttribLayoutRefreshes===1,'attribute layout refreshed more than once without legacy dirtiness');
}
