...
const enableDirtyPresent=src.includes('LWJGL_IMMEDIATE_ATTRIB_ENABLE_DIRTY_V1');
...
if(enableDirtyPresent){
  expect(x.calls.filter(a=>a[0]==='enable').length===3,'second immediate draw should reuse fixed attribute enables');
  expect(c.presentationStats.immediateAttribEnableRefreshes===1,'attribute enables refreshed more than once without legacy dirtiness');
}
