...
for(const marker of ['WEBGL_IMMEDIATE_INTERLEAVED_V1','__LWJGL_IMMEDIATE_INTERLEAVED__','immediateInterleavedUploadsSaved','LWJGL_DETAILED_DRAW_TELEMETRY_OPTIN_V1','detailedDrawTelemetryActive','LWJGL_IMMEDIATE_STABLE_ATTRIB_ENABLE_V1']) expect(src.includes(marker),`missing ${marker}`);
const pointerDirtyPresent=src.includes('LWJGL_IMMEDIATE_POINTER_DIRTY_V1');
const names=['ensureImmediateArrayCapacity','applyCurrentColorAttrib','appendImmediateVertex','Java_org_lwjgl_opengl_GL11_nglBegin','Java_org_lwjgl_opengl_GL11_nglTexCoord2f','Java_org_lwjgl_opengl_GL11_nglVertex3fTexCoord','Java_org_lwjgl_opengl_GL11_nglVertex3f','uploadImmediateInterleaved','Java_org_lwjgl_opengl_GL11_nglEnd'];
...
function make(enabled){
  ...
  const glCtx={ARRAY_BUFFER:0x8892,STATIC_DRAW:0x88E4,FLOAT:0x1406,NO_ERROR:0,
    ...
    disableVertexAttribArray:(...a)=>calls.push(['disable',...a]), vertexAttrib2f:(...a)=>calls.push(['attrib2f',...a]), vertexAttrib4f:(...a)=>calls.push(['attrib4f',...a]), getError:()=>0};
  ...
  detailedDrawTelemetryEnabled: true,
    ...
    immediatePointerLayoutDirty:true, immediateStableAttribEnableDirty:true,
...
expect(x.calls.filter(a=>a[0]==='enable').length===3,'first immediate draw must enable position/color/texcoord');
// Generic current color disables color only; stable position/texcoord enables remain clean.
c.applyCurrentColorAttrib();
expect(c.immediateStableAttribEnableDirty===false,'color-only disable dirtied stable position/texcoord enables');
...
expect(x.calls.filter(a=>a[0]==='enable').length===4,'second draw should re-enable color only');
...
console.log('verify-lwjgl-immediate-interleaved: OK stableAttribEnables=3->1 enabledUploads=1 fallbackUploads=3 stride=36 offsets=0/12/28');