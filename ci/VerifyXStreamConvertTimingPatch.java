...
/** Structural verifier for the diagnostic TreeUnmarshaller conversion timer. */
public final class VerifyXStreamConvertTimingPatch {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 1) throw new IllegalArgumentException("usage: VerifyXStreamConvertTimingPatch candidate.jar");
        final int[] methods = {0};
        final int[] enters = {0};
        final int[] exits = {0};
        final int[] unmarshalCalls = {0};
                        ...
                        return new MethodVisitor(Opcodes.ASM9) {
                            ...
                            public void visitMethodInsn(int opcode, String owner, String n, String d, boolean itf) {
                                ...
                                if (opcode == Opcodes.INVOKEINTERFACE
                                        ...
                                        && "unmarshal".equals(n)) unmarshalCalls[0]++;
                            }
                        };
                    }
                }, 0);
        ...
        if (methods[0] != 1 || enters[0] != 1 || exits[0] != 3 || unmarshalCalls[0] != 1) {
            throw new IllegalStateException("XStream convert timing verification failed methods=" + methods[0]
                    + " enters=" + enters[0] + " exits=" + exits[0] + " unmarshalCalls=" + unmarshalCalls[0]);
        }
        System.out.println("VerifyXStreamConvertTimingPatch: OK methods=1 enters=1 exits=3 unmarshalCalls=1");
    }
}
