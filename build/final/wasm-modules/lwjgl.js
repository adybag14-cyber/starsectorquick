...
// LWJGL_MOUSE_POLL_SNAPSHOT_V1
// Desktop LWJGL exposes non-consuming mouse state from its once-per-frame poll.
// Pack x/y/buttons/inside into one exactly representable JS integer so Java-side
// getX/getY/isButtonDown calls do not cross JNI thousands of times per session.
function Java_org_lwjgl_input_Mouse_nPoll()
{
	inputStats.mousePollSnapshots++;
	var x = Math.max(-32768, Math.min(32767, Math.round(mouseInputState.x))) & 0xffff;
	var y = Math.max(-32768, Math.min(32767, Math.round(mouseInputState.y))) & 0xffff;
	...
	return x + y * 65536 + meta * 4294967296;
}
